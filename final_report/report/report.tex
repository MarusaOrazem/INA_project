...
For finding communities in weighted graph we used Leiden community detection algorithm. Prior to performing the search, we pruned the graphs, so that edges having a value of their weight smaller than the median of all values of weights in a graph. In order to check for any valuable information we may have missed with our algorithm selection, we ran additional experiments using CPM algorithm.

Determining how do communities detected in certain periods of a game differ from standard playing lines required from us to manually input correct formations (line-ups) of teams in observed matches. Since this is a very time-consuming task, we restricted ourselves to 20 matches for which we described the formation in which teams played. Using the parsing program which we developed earlier, we constructed weighted graphs for each of the teams, two graphs representing the state of the game prior and past a certain key event for each team, four in total for a given event in a football match. 



 
\subsection*{Network fragments}
In all experiments concerning network fragments, the networks were pruned before examination. This was done by removing all edges with weights equal to or less than the median weight from the observed graph. The estimation of an adequate pruning point stems from the insights, described in the caption of Figure \ref{BayernWeights}.

To examine the orbit-representation in the observed graphs, an atlas of all possible directed graphlets of node-count 3 or less was created. From there, each unique edge subset from the analysed graph was extracted and a hash table of benchmark orbital roles created. For each subgraph, inferred from the edge subsets, the orbital roles for each contributing node were calculated. The resulting table of this procedure denoted the number of occurences of each player in each of the 34 considered orbital roles. Each network's orbital profile is then calculated as the average number of player-occurences in each orbital role, divided by 11 - the number of players on the pitch. By comparing the differences between these profiles before and after key events, the final insights of the distrbution of PPO (Player per orbit) differences is achieved.




\acknow{The authors would like to acknowledge the help of Doc.  Dr. Lovro Šubelj and Assoc. Prof. Dr. Erik Štrumbelj in the identification of feasible problems within our domain of interest for this course project and for all the help offered in solving them.}
